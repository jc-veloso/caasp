...
vale a pena manter query dupla por dialeto. A saída mais simples é
tirar a data toda do SQL e trazer `ZZC_DTPROC`/`ZZC_HRPROC` como
colunas normais (não são memo, então pode ler direto do resultset, sem
`DbGoto`) e calcular a idade em AdvPL puro — funciona igual em
qualquer banco por trás:

```
cQry := "SELECT ZZC_COD, ZZC_CHVNFE, ZZC_STATUS, ZZC_DTPROC, ZZC_HRPROC, R_E_C_N_O_ AS RECNO FROM " + RetSqlName("ZZC") + " "
cQry += "WHERE ZZC_STATUS IN ('P','A') AND ZZC_PRDPEN = 'N' "
cQry += "AND ZZC_FILIAL = '" + xFilial("ZZC") + "' "
cQry += "AND D_E_L_E_T_ = ' ' "
cQry += "ORDER BY ZZC_DTINCL, ZZC_HRINCL"
...
```
While (cAliZZC)->(!Eof())
    If (cAliZZC)->ZZC_STATUS == "P" .Or. PI_MINATRS((cAliZZC)->ZZC_DTPROC, (cAliZZC)->ZZC_HRPROC) > nMinStale
...
```
Static Function PI_MINATRS(dDtProc, cHrProc)
    Local nSegProc := Val(SubStr(cHrProc,1,2))*3600 + Val(SubStr(cHrProc,4,2))*60 + Val(SubStr(cHrProc,7,2))
Return (Date() - dDtProc) * 1440 + (Seconds() - nSegProc) / 60
```
